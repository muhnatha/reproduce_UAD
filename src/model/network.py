...
class FiLMBlockRand(nn.Module):
    ...
    to focus on the instruction conditioning and amplified on the visual needed to focus
    """
...
class FiLMBlockZero(nn.Module):
    ...
    x = (1 + gamma) * x + beta
    To preserve the initial visual features and only amplify the language conditioning, we initialize gamma and beta to zero, so that the initial output is just x. The model can then learn to amplify or suppress visual features based on the language input.
